...
\section{Payment channel networks}
The Lightning Network is a payment channel network built on top of a blockchain that enables fast and scalable transactions by conducting them off-chain. \\
...
In the Lightning Network's mesh network of payment channels, participants open channels on the blockchain, conduct off-chain transactions, and update the channel balances using commitment transactions. Payments can be routed across the network, enabling trustless and atomic transfers without relying on the underlying blockchain for every transaction.\\
\begin{figure}[t]
\centering
\includegraphics[width=6cm]{network.png}
\caption{mesh network of payment channels}
\end{figure}
...
The output for this Funding Transaction is a single 2-of-2 multisignature script with both participants in this channel. Both participants do not exchange signatures for the Funding
Transaction until they have created spends from this 2-of-2 output refunding the original amount back to its respective funders.
\end{document}